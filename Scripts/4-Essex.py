...
def calculate_steering(S):
    ...
    look_w = max(0.0, min(1.0, (60.0 - fwd) / 35.0))  # 0 on straights, 1 on tight corners
...
def calculate_target_speed(S, R):
    ...
    s_turn = (forward_min < 55.0) and (abs(dir_near) > 0.12) and (abs(dir_far) > 0.12) and (dir_near * dir_far < 0)
...
def apply_brakes(S, R):
    ...
    if forward_min < 28.0 and curvature > 0.28 and s > 70:
        severity = max(0.0, (28.0 - forward_min) / 10.0) + max(0.0, (curvature - 0.28) / 0.12)
...
def drive_modular(c):
    try:
        ...
        forward_min = max(1.0, min(S['track'][8], S['track'][9], S['track'][10]))
        if forward_min > STRAIGHT_SENSOR_MIN and abs(S['angle']) < STRAIGHT_ANGLE_DZ and abs(S['trackPos']) < STRAIGHT_POS_DZ:
            desired = 0.0
