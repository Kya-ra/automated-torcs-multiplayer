...
def drive(c):
    ...
    if speed_x > target_speed:
        brake = clip(
            (speed_x - target_speed) / (25.0 + 10.0 * corner_strength), 0.0, 0.9
        )
        accel = 0.0
    else:
    ...
    stable_for_shift = (
        abs(steer) < SHIFT_BLOCK_STEER and abs(angle) < 0.07 and slip < SHIFT_BLOCK_SLIP
    )
    ...
    stable_now = (
        straightness > 0.68 and abs(steer) < 0.08 and abs(angle) < 0.055 and slip < 1.3
    )
    if stable_now:
    ...
    if can_shift and stable_for_shift:
        if gear == 3:
            ...
            need_down = (speed_x <= DOWN_43) or (
                corner_strength > 0.75 and target_speed < 0.90 * speed_x
            )
